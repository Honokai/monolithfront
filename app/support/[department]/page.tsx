...
export function Page({ params }: { params: { department: string } }) {
  ...
  function handleModalOpening() {
    ...
    // when closing modal values should be forgotten and errors should be cleared
    form.reset()
  }
  ...
  return (
    <div className="w-screen h-screen px-4">
      <Dialog open={showModal} onOpenChange={handleModalOpening}>
                              ...
                              <ScrollArea className="h-80 rounded-md">
                                {
                                  ticket.responses && ticket.responses.map(response => {
                                    return (
                                      <div key={response.id} className="flex flex-col bg-muted p-2 rounded my-1 gap-1">
                                        <div className="flex text-sm" aria-level={1} role="heading">
                                          <div className="flex-1">User name</div>
                                          <div className="flex-1 flex justify-end">{moment.utc(response.created_at).format("DD/MM/YYYY HH:mm:ss")}</div>
                                        </div>
                                        <div className="p-2 text-sm bg-background rounded">
                                          {response.inquiry}
                                        </div>
                                      </div>
                                    )
